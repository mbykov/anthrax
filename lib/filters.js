...
// // verb or name - ἄβακος, βαίνω, βέβηκα,
export function simple(doc, flex) {
    let chain = false
    if (doc.name && flex.key == doc.key) chain = true
    else if (doc.verb && doc.keys.find(key=> flex.key == key.key)) chain = true
    if (chain) return [doc, flex]
}

...
export function prettyRes(chain) {
    let prettyres = {}
    ...
    let rdict = mainseg.rdict
    prettyres.rdict = rdict

    let cdict = mainseg.cdicts[0]

    prettyres.dname = cdict.dname
    ...
    else if (cdict.name) prettyres.fls = prettyNameFLS(flsseg.fls)
    if (prettyres.fls) prettyres.fls = JSON.stringify(_.uniq(prettyres.fls).sort())
    // prettyres.trns = cdict.trns[0]
    if (!prettyres.pref) delete prettyres.pref
    return prettyres
}

function prettyVerbFLS_(fls) {
    return fls.map(flex=> {
        let str
